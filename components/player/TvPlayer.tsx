...
function TvPlayer({ tvId, season = 1, episode = 1, title }: Props) {
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 4px' }}>
        <span style={{ fontSize: 12, color: 'var(--muted)' }}>Season {season} · Episode {episode}</span>
      </div>
      ...
      <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', background: '#000', borderRadius: 16, overflow: 'hidden', boxShadow: '0 0 40px rgba(0,0,0,0.6), inset 0 0 0 1px rgba(255,255,255,0.06)' }}>
        {shouldRender && (
          <iframe
            key={SOURCES[sourceIdx].url(tvId, season, episode)}
            src={SOURCES[sourceIdx].url(tvId, season, episode)}
            title={title ? `${title} S${season}E${episode} — SoarTV` : `TV S${season}E${episode} — SoarTV`}
        ...
        <ClickShield resetKey={`${tvId}-S${season}E${episode}-${sourceIdx}`} title={title ? `Play ${title} · S${season}E${episode}` : 'Tap to play'} />
      </div>
    </div>
  )
}
